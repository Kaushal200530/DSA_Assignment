...
class Student{
    ...
    public:
        ...
        void insertionSort(Student s[], int n){
            for(int i=1; i<=n; i++){
                int j=i-1; // j is the previous key 
                int key = s[i].roll; // current roll number is key 
                while(j>=0 && s[j].roll > key){
                    s[j+1].roll = s[j].roll;  // move the larger key to the right side make the space for the key
                    j--;
                }
                s[j+1].roll = key;
            } 
...
int main(){
    Student s[3], x;
    int n=sizeof(s)/ sizeof(s[0]); //this is used to calculate the number of element in the array 
    int choice;
    do{
